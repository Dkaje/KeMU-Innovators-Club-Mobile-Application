...
import com.example.kemussit.Kitendawili.Shughuli;
...
public class CoolClub extends AppCompatActivity {
    String myThme, myVenu, myClub, myCoun, mySum;
    Shughuli cardAda;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent() != null) {
            ...
            myCoun = getIntent().getStringExtra("counter");
        }
        Objects.requireNonNull(getSupportActionBar()).setTitle(myClub + "-" + myVenu);
        setContentView(R.layout.activity_cool_club);
    ...
    private void getCard() {
        ...
        requestQueue.add(new StringRequest(Request.Method.POST, Connect.getAmri,
                response -> {
                    try {
                        ...
                        if (success == 1) {
                            ...
                            for (int i = 0; i < jsonArray.length(); i++) {
                                ...
                                mySum = jsonObject.getString("money");
                            ...
                            cardAda = new Shughuli(CoolClub.this, R.layout.roller, cardModeArrayList);
                            ...
                            sender.setOnClickListener(view -> {
                                String app = String.format("%.0f", Float.parseFloat(myCoun) * Float.parseFloat(mySum));
                                builder = new AlertDialog.Builder(this);
                                builder.setTitle("Confirm Club Participation");
                                builder.setMessage(Html.fromHtml("<font color='#000000'><b><strong>Club</strong></b> :" + myClub + "<br><b><strong>Members</strong></b> :" + myCoun + "<br><b><strong>amountPerMember</strong></b>: Kes" + mySum + "<br><b><strong>FullWithdrawal</strong></b>: Kes" + app + "</font>"));
                                builder.setPositiveButton("Cancel", (dialogInterface, iu) -> {
                                });
                                builder.setNeutralButton(Html.fromHtml("<font color='#ff0000'><b><u>Click_Here</b></font>"), (dialogInterface, iu) -> {
                                });
                                alertDialog = builder.create();
                                alertDialog.show();
                                alertDialog.setCancelable(false);
                                ...
                                alertDialog.getButton(DialogInterface.BUTTON_NEUTRAL).setOnClickListener(view1 -> {
                                    ...
                                    nemis.setMessage("Hi " + model.getFname() + ",\nYou are just about to notify the Treasurer over the withdrawal of\nKES" + app + "\nfor the " + myVenu + " event scheduled on\ndate " + cardMode.getDate() + " " + cardMode.getTime() + ".");
                                    nemis.setPositiveButton("Yes_Alert!", (dialogInterface, iu) -> {
                                    });
                                    ...
                                    keybo.getButton(DialogInterface.BUTTON_POSITIVE).setOnClickListener(view8 -> {
                                        requestQueue = Volley.newRequestQueue(getApplicationContext());
                                        requestQueue.add(new StringRequest(Request.Method.POST, Connect.determined,
                                                respo -> {
                                                    try {
                                                        jsonObject = new JSONObject(respo);
                                                        int st = jsonObject.getInt("demo");
                                            ...
                                            protected Map<String, String> getParams() throws AuthFailureError {
                                                ...
                                                para.put("venu", myVenu);
                                                para.put("venue", myVenu + "/" + cardMode.getLand() + "/" + cardMode.getSite());
                                                para.put("date", cardMode.getDate() + " " + cardMode.getTime());
                                                para.put("club", myClub);
                                                para.put("org_no", cardMode.getOrg_no());
                                                para.put("org_phone", cardMode.getOrg_phone());
                                                para.put("org_name", cardMode.getOrg_name());
                                                para.put("pat_id", model.getStud_id());
                                                para.put("pat_phone", model.getPhone());
                                                para.put("pat_name", model.getFname());
                                                para.put("members", myCoun);
                                                para.put("money", mySum);
                                                para.put("summ", app);
