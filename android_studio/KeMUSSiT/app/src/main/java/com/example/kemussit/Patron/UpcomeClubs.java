...
import com.example.kemussit.Orggen.ViewOldage;
...
public class UpcomeClubs extends AppCompatActivity {
    SahauAda eventAda;
    SahauMode evenMdel;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Objects.requireNonNull(getSupportActionBar()).setTitle("New Club Events");
        setContentView(R.layout.activity_upcome_clubs);
        ...
        listView.setOnItemClickListener((adapterView, view, i, l) -> {
            ...
            startActivity(new Intent(getApplicationContext(), CoolClub.class).putExtra("theme", evenMdel.getTheme()).putExtra("venue", evenMdel.getVenue()).putExtra("club", evenMdel.getClub()).putExtra("counter", evenMdel.getCounter()));
        });
    }

    private void getCard() {
        requestQueue = Volley.newRequestQueue(getApplicationContext());
        requestQueue.add(new StringRequest(Request.Method.POST, Connect.kibosh,
                response -> {
                    try {
                        ...
                        if (success == 1) {
                            ...
                            eventAda = new SahauAda(UpcomeClubs.this, R.layout.russia, evenMdelArrayList);
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()) {
            case R.id.Memes:
                startActivity(new Intent(getApplicationContext(), Salome.class));
                break;
            default:
        }
