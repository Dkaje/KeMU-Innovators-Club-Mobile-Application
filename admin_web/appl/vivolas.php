...
function test_input($data)
{
    ...
    if (preg_match($caps, $mpesa)) {
        ...
        }elseif (mysqli_num_rows(mysqli_query($con, "SELECT * FROM cardy_one WHERE deter='R' and finsta=1 and secsta=0 and reg_no='$reg_no'")) > 0) {
            $response["success"] = 0;
            $response["message"] = "Failed!! You have a card under renewal processing";
        }elseif (mysqli_num_rows(mysqli_query($con, "SELECT * FROM cardy_one WHERE deter='R' and finsta=1 and secsta=1 and reg_no='$reg_no' and lost=0")) > 0) {
            $response["success"] = 0;
    ...
    $iden=$h[0];
            ...
            VALUES('$iden','R','$mpesa','$fees','$reg_no','$fullname','$phone','$date','$term','$year','$ses')")) {
        $profile = mysqli_fetch_assoc(mysqli_query($con, "SELECT profile as img from issued where reg_no='$reg_no'"));
        $signature = mysqli_fetch_assoc(mysqli_query($con, "SELECT signature as img from issued where reg_no='$reg_no'"));
                ...
                VALUES('$iden','R','$category','$reg_no','$fullname','$phone','$date','$profile[img]','$signature[img]','$department','$classification','$email','$ses')")){
$response["success"] = 1;
