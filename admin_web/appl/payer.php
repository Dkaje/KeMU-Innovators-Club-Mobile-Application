...
function test_input($data)
{
    ...
    if (preg_match($caps, $mpesa)) {
        ...
        }elseif (mysqli_num_rows(mysqli_query($con, "SELECT * FROM cardy_one WHERE deter='F' and finsta=1 and secsta=0 and reg_no='$reg_no'")) > 0) {
            $response["success"] = 0;
            $response["message"] = "Failed!! You have a card under processing";
        }elseif (mysqli_num_rows(mysqli_query($con, "SELECT * FROM cardy_one WHERE deter='F' and finsta=1 and secsta=1 and reg_no='$reg_no' and lost=0")) > 0) {
            $response["success"] = 0;
            $response["message"] = "Failed!! You were issued a card whose expiry is yet.";
        }elseif (mysqli_num_rows(mysqli_query($con, "SELECT * FROM issued WHERE status=1 and ended=1 and expiry=1 and reg_no='$reg_no'")) > 0) {
            $response["success"] = 0;
            $response["message"] = "Failed!! Our Record shows you have an EXPIRED card. Please Renew it.";
    ...
    $iden=$h[0];
            ...
            VALUES('$iden','F','$mpesa','$fees','$reg_no','$fullname','$phone','$date','$term','$year','$ses')")) {
                ...
                VALUES('$iden','F','$category','$reg_no','$fullname','$phone','$date','$filenameprofile','$filesignature','$department','$classification','$email','$ses')")){
