        ...
        if($status==1){
            if (mysqli_query($con, "UPDATE payment set status='$status' where pay='$pay'")) {
                ...
                if(mysqli_num_rows(mysqli_query($con,"SELECT * from account"))>0){
                    if(mysqli_query($con,"UPDATE account set fresh=(fresh+$fresh),whole=(whole+$fresh),bal=(bal+$fresh),last='$reg_date'")){
                    ...
                    if(mysqli_query($con,"INSERT into account(whole,fresh,bal,entry,last)values('$fresh','$fresh','$fresh','$reg_date','$reg_date')")){
                        mysqli_query($con, "UPDATE cardy_one set finsta='$finsta' where pay='$pay'");
                        $response["success"] = 1;
                        $response["message"] = "payment Approved successfully";
                    }else{
