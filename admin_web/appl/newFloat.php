...
function test_input($data)
{
    ...
    fclose($em);
    $iden=$h[0];
            if (mysqli_query($con, "INSERT INTO contribution(pay,mpesa,money,reg_no,fullname,phone,date,term,sm,ses)
            VALUES('$iden','$mpesa','$money','$reg_no','$fullname','$phone','$date','$term','$sm','$ses')")) {
        mysqli_query($con,"UPDATE summer set pay='$pay' where sm='$sm'");
$response["success"] = 1;
$response["message"] = "Event payment was successful";
                }else{
                    $response["success"] = 0;
